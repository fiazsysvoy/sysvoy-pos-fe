...
const DashBoard = () => {
  ...
  const [revenueStats, setRevenueStats] = useState({
    ...
    historicalMonthlyData: [] as Array<{
      month: string;
      revenue: number;
      sales: number;
    }>,
  ...
  const fetchRevenueStats = async () => {
    try {
      ...
      toast.error(
        err.message || "Something went wrong fetching revenue stats.",
      );
    }
  };
  ...
  const weekStartFormatted = formatDate(
  ...
  const prepareTodayChartData = (
    hourlyData: Array<{ time: string; revenue: number }>,
  ) => {
  ...
  const prepareMonthChartData = (
    weeklyData: Array<{ date: string; revenue: number; week?: number }>,
  ) => {
